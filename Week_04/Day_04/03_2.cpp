...
int main() {
	ParentClass* a = new ChildClass();  //szulo tipusu valtozo, amivel mutatunk a child classra
	...
	a->setVariable(2); //a szulo setVariable fuggvenyet hivta be
	((ChildClass*)a)->setVariable(4);  //gyerek tipuskent kezeld az 'a' valtozot, ami szulo osztalyban van, de a gyerekkent kezelt 'a'-nak a setVariable fuggvenyet hivjuk meg
	cout << "See the resulting variables for Yourself:" << endl;
	cout << a->getVariable() << "  " << ((ChildClass*)a)->getVariable() << endl;
	cout << "Now let's see how we can deal with them still by referencing them through their original calsses' namespace:" << endl;
